...
compare_blocks(lzma_block *block_expected, lzma_block *block_actual)
{
	...
	assert_lzma_ret(block_actual->filters[filter_count].id,
			LZMA_VLI_UNKNOWN);
}
...
test_lzma_block_header_decode(void)
{
	...
	assert_lzma_ret(lzma_block_header_decode(&decode_block, NULL, out),
			LZMA_OK);
	...
	// Test bad CRC type
	decode_block.check = LZMA_CHECK_ID_MAX + 1;
	assert_lzma_ret(lzma_block_header_decode(&decode_block, NULL, out),
			LZMA_PROG_ERROR);
	decode_block.check = LZMA_CHECK_CRC32;
